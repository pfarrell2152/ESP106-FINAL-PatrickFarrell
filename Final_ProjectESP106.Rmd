...
## Although I am not working wit all of the data frames in this project I wrote a for loop that is able to make this plot for all soil measuremetns in a growing region. To complete it i could ad a list for each measurement along with the desired name and y-axis label.

```{r}
# List of data frames and corresponding plot names
Grower_comparison<- list(
  list(df = LODI_pH_sum_df, plot_name = "pH_Grower_plot_LODI", y_label = "pH"),
  list(df = pH_sum_df, plot_name = "pH_Grower_plot_PASO", y_label = "pH"),
  list(df = NAPA_pH_sum_df, plot_name = "pH_Grower_plot_NAPA", y_label = "pH")
)


for (item in Grower_comparison) {
  plot <- ggplot(item$df, aes(y = !!sym(paste0(item$y_label, "_mean")), x = Grower, fill = Depth)) +
    geom_boxplot(outlier.shape = NA) +
    theme_bw() +
    scale_fill_manual(values = cbPalette) + 
    theme(axis.text.x = element_text(hjust = 0.5, angle = 45, size = 12),
      ...
      legend.text = element_text(size = 12),
      legend.title = element_text(size = 12))+
    ylab("pH")+ ggtitle(item$plot_name)

 
  assign(item$plot_name, plot)
}

# Display the plots
print(pH_Grower_plot_LODI)

print(pH_Grower_plot_PASO)

print(pH_Grower_plot_NAPA)
...
Now we to compare soil factors across depths and across reps for each vineyard I will create a box plots for each vineyard. 

```{r}
#the following code set a colorblind friendly color palette for the figures

cbPalette <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")


create_EC_plot <- function(data, ideal_values, challenging_values) {
  ggplot(data, aes(y = EC_mean, x = Grower, fill = Depth)) + 
    geom_boxplot(outlier.shape = NA) +
    theme_bw() + 
    scale_fill_manual(values = cbPalette) +
    theme(
    ...
    ylab(expression(paste('EC (uS/cm)'))) + 
    geom_jitter(
      aes(color = as.factor(ideal_challenging)), 
      size = 3,
...
# Convert EC_sum_df to a data frame
EC_sum_df <- as.data.frame(EC_sum_df)


# Create and save plots for each grower
unique_growers <- unique(EC_sum_df$Grower)
for (grower in unique_growers) {
  data <- subset(EC_sum_df, Grower == grower)
  plot <- create_EC_plot(data, ideal_values[[grower]], challenging_values[[grower]])
  ggsave(paste0(grower, "_ECplot.pdf"), plot, width = 6, height = 8)
}
...
```{r}
CIR_pHplot<- ggplot(pH_sum_df,aes(y=pH_mean, x=Location, fill=Depth)) + geom_boxplot(outlier.shape=NA) +theme_bw() + scale_fill_manual(values=cbPalette) + 
  theme(axis.text.x=element_text (hjust=0.5, size = 12), axis.title.x = element_text(size=14), axis.title.y = element_text(size=14), plot.title = element_text(size=18), strip.text.x = element_text(size = 12),strip.text.y = element_text(size = 12), axis.text.y.left =element_text( size = 12), legend.text=element_text(size=12), legend.title=element_text(size=12)) + 
  ylab(expression(paste('pH' ))) + geom_jitter(size=3, shape=16,position=position_jitterdodge(), color=ifelse(pH_sum_df$pH_mean==c(6.75, 6.443333, 7.13, 7.443333), "green", ifelse(pH_sum_df$pH_mean==c(6.98,7.02, 8.116666667, 8.53), "red", "black")))
